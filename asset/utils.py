...
class goPublish:
            ...
            for p in self.svnInfo:
                if p.project.name == self.name:
                    ...
                    self.saltCmd.cmd('%s' % s.saltminion.saltname, 'cmd.run',
                                     ['mv %s %s/%s_%s' % (p.executefile, p.movepath, self.name, currentTime)])
                    # deploy go project
                    svn = self.saltCmd.cmd('%s' % s.saltminion.saltname, 'cmd.run',
                                           [
                                               'svn update -r%s --username=%s --password=%s --non-interactive %s && svn log -l 1 --username=%s --password=%s --non-interactive %s'
                                               % (self.svn_revision, p.username, p.password, p.localpath, p.username,
                                                  p.password, p.localpath)])
                    result.append(svn)
        ...
        if self.svn_revision == 'head':
            ...
            if not get_service_status(services):
                ...
                if goservice_rev_last:
                    revert_info = {
                        'Warning': '#####################Roll back to the previous version######################'}
                    result.append(revert_info)
                    result += self.deployGo(self.name, self.services, self.username, self.ip, self.tower_url,
                                            self.phone_number, svn_revision=goservice_rev_last[0],
        ...
        for p in self.svnInfo:
            if p.project.name == self.project:
                ...
                revertResult = commands.getstatusoutput(
                    "salt '%s' cmd.run 'cp %s/%s %s'" % (self.host, p.movepath, self.revertFile, p.executefile))

                if revertResult[0] == 0:
                    for obj in goservices.objects.filter(env=self.env):
                        if obj.group.name == self.project and self.host == obj.saltminion.saltname:
                            restart = self.saltCmd.cmd('%s' % self.host, 'cmd.run',
    ...
    def build_go(self, hostname, project, supervisorName, goCommand, svnRepo, svnUsername, svnPassword, fileName,
        ...
        # })
        try:
            pillar = " pillar=\"{'goprograme': '%s','supProgrameName': '%s','goRunCommand': '%s','svnrepo': '%s','svnusername': '%s','svnpassword': '%s'}\"" % (
            self.project, self.supervisorName, self.goCommand, self.svnRepo, self.svnUsername, self.svnPassword)
            ...
            for host, info in r.items():
                ...
                try:
                    if not Asset.objects.filter(hostname=hostname_id):
                        Asset.objects.create(ip=ip, hostname=hostname_id, system_type=os, cpu=cpu,
                                             memory=str(memory) + 'M', asset_type=asset_type)
...
class go_monitor_status(object):
    ...
    def get_supervisor_status(self, hostname_id):
        ...
        for info in obj:
            try:
                s = xmlrpclib.Server('http://%s:%s@%s:%s/RPC2' % (
                info.supervisor_username, info.supervisor_password, info.supervisor_host, info.supervisor_port))
...
class crontab_svn_status(object):
    ...
    def crontab_svn_update(self, hostname, project, phone_number):
        ...
        cmd = ["svn checkout %s %s --username=%s --password=%s --non-interactive " % (
        obj.repo, obj.localpath, obj.username, obj.password), 'env={"LC_ALL": "en_US.UTF-8"}']
        print 'cmd : '
        print cmd
        data = {
            'client': 'local',
...
@task
def deploy_go(env, hostname, project, supervisorName, goCommand, svnRepo, svnUsername, svnPassword, fileName, username,
              ip):
    obj = goPublish(env)
    obj.build_go(hostname, project, supervisorName, goCommand, svnRepo, svnUsername, svnPassword, fileName, username,
                 ip)
...
def dingding_robo(hostname='', project='', result='', username='', phone_number='', types=1, info=''):
    ...
    if types == 1:
        content = current_time + " " + errmsg + ": " + "deploy " + str(project) + " to " + str(hostname) + " by " + str(
            username)
    elif types == 2:
        content = current_time + ": " + info
    elif types == 3:
        content = current_time + " " + errmsg + ": " + "revert " + str(project) + " to " + str(hostname) + " by " + str(
            username)
